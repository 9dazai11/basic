...
class SiteController extends Controller
{
    ...
    public function actionResendVerificationEmail()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                Yii::$app->session->setFlash('success', 'Проверьте свой адрес электронной почты для получения дальнейших инструкций.');
                return $this->goHome();
            }
            Yii::$app->session->setFlash('error', 'Извините, мы не можем повторно отправить подтверждающее письмо на указанный адрес электронной почты.');
        }
